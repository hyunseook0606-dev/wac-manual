...
}[] = [
  {
    ...
    steps: [
      ...
      },
      {
        ...
        title: "거래처 검색",
        body: [
          "거래처 칸에서 상호 일부를 검색합니다.",
          "예: 몽콕 아웃닭이면 「아웃닭」만 쳐도 됩니다.",
          "검색 결과에 몽콕·침사추이처럼 지점이 같이 뜨면, 주문 지점에 맞는 줄을 고릅니다.",
        ],
      ...
      },
      {
        ...
        body: [
          "출하창고는 「본사창고」로 둡니다.",
          "우리가 피킹하는 곳이 본사창고입니다. 창고 메뉴얼의 도면(A/B/R)도 이 본사창고입니다.",
          "「팝업」은 사무실 데스크에 있는 재고입니다. 피킹용으로 쓰지 않습니다.",
        ],
      ...
      },
      {
        ...
        title: "특이사항 두 칸",
        body: [
          "특이사항은 배송용 / 창고용이 따로 있습니다. 누구에게 보이는지가 다릅니다.",
        ],
        example: [
          {
            label: "특이사항(배송)",
          ...
          {
            label: "특이사항(창고)",
            text: "창고에서 피킹하는 사람만 · 예: 단무지상태 체크후 피킹하세요",
          },
        ],
        img: "/ecount/06-item-filled.png",
        imgCap: "배송·창고 특이사항이 채워진 예시",
      },
      {
        no: "06",
        title: "품목 넣고 수량만",
        body: [
          "품목코드 칸에 핵심 단어만 쳐도 됩니다.",
          "예: 「고추가루김치용」이 필요하면 「김치」만 검색 → 굵은고추가루(김치용) 선택",
          "선택되면 품목·단가가 들어가고, 수량만 적으면 됩니다.",
        ],
        tip: "정확한 품목명을 전부 외울 필요 없습니다. 키워드로 찾고 클릭.",
  ...
  return (
    <div className="ecount-page">
      <div className="rule-callout">
        <p>
          주문 들어온 품목을 보고{" "}
          <strong>재고 I → 판매 → 판매입력</strong>에서 입력합니다. 아래 순서대로
          따라가면 됩니다.
        </p>
      </div>
      ...
      {page.steps.map((step) => (
        <article key={step.no} className="ecount-step">
          ...
          {step.tip ? <p className="rule-note">{step.tip}</p> : null}
          {step.example ? (
      ...
      <p className="rule-note rule-note-strong">
        여기까지: 판매입력 화면 열고 → 거래처·온라인·본사창고·특이사항 → 품목
        검색 후 수량 입력
      </p>
    </div>
  );
